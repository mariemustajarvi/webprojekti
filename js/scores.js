...
// testaus
const canAccessBonusGame = () => {
    const requiredGames = ['verbi', 'substantiivi', 'adjektiivi', 'numeraali', 'pronomini'];
    // Tarkista, onko kaikki pääpelit läpäisty
    return requiredGames.every(game => gameIsBeaten(game));
};

const updateBonusGameLink = () => {
    const bonusLink = document.getElementById("bonus-link");

    if (canAccessBonusGame()) {
        // Aktivoi bonuspelin linkki
        bonusLink.classList.remove("disabled");
        bonusLink.removeAttribute("onclick");
    } else {
        // Estä bonuspelin käyttö
        bonusLink.classList.add("disabled");
        bonusLink.setAttribute("onclick", "return false;");
    }
};

// Tarkista bonuspelin tila sivun latauksessa
document.addEventListener("DOMContentLoaded", updateBonusGameLink);
